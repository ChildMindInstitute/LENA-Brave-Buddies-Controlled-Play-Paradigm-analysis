##Brave Buddies Full Day Totals ANOVA##
#Jacob Stroud
...
write.csv(durAll, "durAllTotal.csv")

durAllTotal <- read.csv("durAllTotal.csv")

days <- c(1, 2, 3, 4)
daysFactor <- as.factor(days)
daysFrame <- data.frame(daysFactor)
durBind <- cbind(durAllTotal$V1, durAllTotal$V2, durAllTotal$V3, durAllTotal$V4)
...
write.csv(segAll, "segAllTotal.csv")

segAllTotal <- read.csv("segAllTotal.csv")

days <- c(1, 2, 3, 4)
daysFactor <- as.factor(days)
daysFrame <- data.frame(daysFactor)
segBind <- cbind(segAllTotal$V1, segAllTotal$V2, segAllTotal$V3, segAllTotal$V4)
...
write.csv(wordsAll, "wordsAllTotal.csv")

wordsAllTotal <- read.csv("wordsAllTotal.csv")

days <- c(1, 2, 3, 4)
daysFactor <- as.factor(days)
daysFrame <- data.frame(daysFactor)
wordsBind <- cbind(wordsAllTotal$V1, wordsAllTotal$V2, wordsAllTotal$V3, wordsAllTotal$V4)
...
write.csv(turnsAll, "turnsAllTotal.csv")

turnsAllTotal <- read.csv("turnsAllTotal.csv")

days <- c(1, 2, 3, 4)
daysFactor <- as.factor(days)
daysFrame <- data.frame(daysFactor)
turnsBind <- cbind(turnsAllTotal$V1, turnsAllTotal$V2, turnsAllTotal$V3, turnsAllTotal$V4)
