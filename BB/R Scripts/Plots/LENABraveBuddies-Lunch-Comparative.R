#LENA Data Analysis: Lunch time comparative data
#Jacob Stroud
...
png(filename ="/Volumes/data/Research/CDB/Progress Monitoring:LENA/Brave Buddies/Plots and Results/child-voc-groups_lunch_comparative.png",width=1400, height=600) #Opens a PNG file#
...
plot(c(0,length(days)+1), c(0, 20), type="n" , xlab ="Day", main = "Normalized Child Vocalizations During Lunchtime", cex.main=2, xaxt = "n",ylab = "Vocalizations per 5 minute block", cex.lab=1.3)
...
for (i in 1:length(sub_folders_paths) ){
  ...
  } else if  (identical(sub_folders[i], "M00475465") || identical(sub_folders[i], "M00413464") || identical (sub_folders[i],"M00447059") || identical (sub_folders[i],"M00490907") || identical(sub_folders[i],"M00495999") || identical(sub_folders[i],"M00472399" )) {
    groupColor="blue"
  }
  
  
  dayfiles=dir(pattern=paste(sub_folders[i], "_Lunch_Comparative.csv", sep = ""), full.names = FALSE)
  ...
  points(x= 1:length(days), y= bravebuds_counts$Child_Voc_Count_Normalized[1:length(days)], pch=19, col= groupColor,type = "o",lty=3)
  wordsAll=rbind(wordsAll, bravebuds_counts$Child_Voc_Count_Normalized[1:length(days)])
  for (j in 1:length(days)) {
    day_diff_words[i,j] =bravebuds_counts$Child_Voc_Count_Normalized[j]-bravebuds_counts$Child_Voc_Count_Normalized[1]
    colorcheck[i,j]=bravebuds_counts$Child_Voc_Count_Normalized[j]-bravebuds_counts$Child_Voc_Count_Normalized[1]
  }
...
#progress
plot(c(0,length(days)), c(-10,10), type="n" , main ="Child's Vocalization Progress During Lunch time", cex.main=2, xaxt = "n", xlab=NA, ylab = "Modulations in child vocalizations compared to Monday", cex.lab=1.3, cex.main=2)
...
for (q in 1:dim(day_diff_words)[1]){
  ...
  } else if  (identical(sub_folders[q], "M00475465") || identical(sub_folders[q], "M00413464") || identical (sub_folders[i],"M00447059") || identical (sub_folders[q],"M00490907") || identical(sub_folders[q],"M00495999") || identical(sub_folders[q],"M00472399" )) {
    groupColor="blue"
  }
...
png(filename ="/Volumes/data/Research/CDB/Progress Monitoring:LENA/Brave Buddies/Plots and Results/child-voc-dur-group-Lunch_comparative.png",width=1400, height=600)
par(mfrow=c(1,2))
...
plot(c(0,length(days)+1), c(0, 10), type="n" , xlab ="Day", main = "Child Vocalization Duration During Lunch per 5 Minute Block", cex.main=2, xaxt = "n",ylab = "Time (s)", cex.lab=1.3)
axis(side = 1,at= c(1:length(days)),labels =days,  las=1)
...
for (i in 1:length(sub_folders_paths) ){
  ...
  points(x= 1:length(days), y= bravebuds_counts$Child_Voc_Duration_Normalized[1:length(days)], type = "o", pch=19, col= groupColor,lty=3)
  #sum_durs= sum_durs+bravebuds_counts$Child_Voc_Duration_Normalized
  durAll=rbind(durAll, bravebuds_counts$Child_Voc_Duration_Normalized[1:length(days)])
  
  for (j in 1:length(days)) {
    day_diff_durs[i,j] =bravebuds_counts$Child_Voc_Duration_Normalized[j]-bravebuds_counts$Child_Voc_Duration_Normalized[1]
    colorcheck[i,j]=    bravebuds_counts$Child_Voc_Duration_Normalized[j]-bravebuds_counts$Child_Voc_Duration_Normalized[1]
  }
  colorcheck[i,length(days)+1]=dayfiles
...
plot(c(0,length(days)), c(-5, 5), type="n" , main ="Child's Vocalization Duration Progress During Lunch", cex.main=2, xaxt = "n", xlab=NA,ylab = "Modulations in child's vocalization duration progress compared to monday lunch", cex.lab=1.3, cex.main=2)
axis(side = 1,at= 1:3,labels = c( "Tue-Mon", "Wed-Mon", "Thur-Mon"),  las=1)
...
png(filename ="/Volumes/data/Research/CDB/Progress Monitoring:LENA/Brave Buddies/Plots and Results/child-conv-turns-group-lunch-comparative.png",width=1400, height=600)
par(mfrow=c(1,2))
...
plot(c(0,length(days)+1), c(0, 10), type="n" , xlab ="Day", main = "Conversational Turns During Lunchtime", cex.main=2, xaxt = "n",ylab = "Turns per 5 Minute Block"  , cex.lab=1.3)
axis(side = 1,at= c(1:length(days)),labels =days,  las=1)

for (i in 1:length(sub_folders_paths) ){
  setwd(paste(sub_folders_paths[i], "/Lunchtime View", sep = ""))
  dailyFile=dir(pattern="_Lunch_Comparative", full.names = FALSE)
  ...
  points(x= 1:length(days), y= bravebuds_counts$Turn_Count_Normalized[1:length(days)], type = "o",lty=3, pch=19, col= groupColor)
  turnsAll=rbind(turnsAll, bravebuds_counts$Turn_Count_Normalized[1:length(days)])
  
  #sum_turns= sum_turns+bravebuds_counts$Turn_Count_Normalized
  for (j in 1:length(days)) {
    day_diff_turns[i,j] =bravebuds_counts$Turn_Count_Normalized[j]-bravebuds_counts$Turn_Count_Normalized[1]
    colorcheck[i,j]=    bravebuds_counts$Child_Voc_Duration_Normalized[j]-bravebuds_counts$Child_Voc_Duration_Normalized[1]
  }
...
plot(c(0,length(days)), c(-5,5), type="n" , main ="Child's conversational turns Progress", cex.main=2, xaxt = "n", xlab=NA,ylab = "Modulations in child's conversational turns progress as off Monday's", cex.lab=1.3, cex.main=2)
axis(side = 1,at= 1:3,labels = c( "Tue-Mon", "Wed-Mon", "Thur-Mon"),  las=1)
...
png(filename ="/Volumes/data/Research/CDB/Progress Monitoring:LENA/Brave Buddies/Plots and Results/child-segment-dur-group-lunch-comparative.png",width=1400, height=600)
par(mfrow=c(1,2))
...
plot(c(0,length(days)+1), c(0, 20), type="n" , xlab ="Day", main = "Segment Duration per 5 Minute Block During Lunch", cex.main=2, xaxt = "n",ylab = "Time (s)"  , cex.lab=1.3)
axis(side = 1,at= c(1:length(days)),labels =days,  las=1)


for (i in 1:length(sub_folders_paths) ){
  setwd(paste(sub_folders_paths[i], "/Lunchtime View", sep = ""))
  dailyFile=dir(pattern="_Lunch_Comparative.csv", full.names = FALSE)
  bravebuds_counts= read.csv(dailyFile)
  if (identical(sub_folders[i], "M00445929")) {
    groupColor="red"
  } else if (identical(sub_folders[i], "M00412434") || identical( sub_folders[i],"M00441664") || identical(sub_folders[i],"M00402344")|| identical(sub_folders[i],"M00440011") || identical( sub_folders[i],"M00494954" ) ){
...
plot(c(0,length(days)), c(-10,10), type="n" , main ="Child's Segment Duration Progress", cex.main=2, xaxt = "n", xlab=NA,ylab = "Modulations in  child's segment duration compared to monday lunch", cex.lab=1.3, cex.main=2)
axis(side = 1,at= 1:3,labels = c( "Tue-Mon", "Wed-Mon", "Thur-Mon"),  las=1)
