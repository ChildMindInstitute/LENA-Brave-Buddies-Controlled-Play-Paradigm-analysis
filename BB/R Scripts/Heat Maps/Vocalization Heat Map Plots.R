#LENA Data Analysis: Vocalization Heat Map
#Jacob Stroud
...
##1 Section PNG##
png(filename ="/Volumes/data/Research/CDB/Progress Monitoring:LENA/Brave Buddies/Plots and Results/Monday-voc-HeatMap.png",width=1400, height=600) #Opens a PNG file#
...
##Plot##
plot(c(0,length(activities)+1), c(0, 30), type="n" , xlab ="Activity", main = "Child Vocalizations on Monday", cex.main=2, xaxt = "n",ylab = "Average vocalizations per 5 minute segment", cex.lab=1.3)
...
for (i in 1:length(sub_folders_paths) ){
  ...
  dayfiles=dir(pattern=paste(sub_folders[i], "_Vocalization.csv", sep = ""), full.names = FALSE)
...
arrows(x0=1:5, y0=apply(wordsAll,2,mean), x1=1:5, y1=apply(wordsAll,2,mean)-apply(wordsAll,2,sd)/sqrt(length(sub_folders)),length=.05,angle = 90,lty=1,lwd=2)
legend(x=0, y= 25, c("group 1 ", "group 2", "group 3"), lty=0 ,col = c("red", "green", "blue") ,pch=20, text.font = 10, bty = "n", cex = 1)

dev.off()

####### Tuesday #######

png(filename ="/Volumes/data/Research/CDB/Progress Monitoring:LENA/Brave Buddies/Plots and Results/Tuesday-voc-HeatMap.png",width=1400, height=600)
...
plot(c(0,length(activities)+1), c(0, 30), type="n" , xlab ="Day", main = "Child Vocalizations on Tuesday", cex.main=2, xaxt = "n",ylab = "Average vocalizations per 5 minute segment", cex.lab=1.3)
axis(side = 1,at= c(1:length(activities)),labels =activities,  las=1)
...
png(filename ="/Volumes/data/Research/CDB/Progress Monitoring:LENA/Brave Buddies/Plots and Results/Wednesday-voc-HeatMap.png",width=1400, height=600)
...
plot(c(0,length(activities)+1), c(0, 30), type="n" , xlab ="Day", main = "Child Vocalizations on Wednesday", cex.main=2, xaxt = "n",ylab = "Average vocalizations per 5 minute segment"  , cex.lab=1.3)
axis(side = 1,at= c(1:length(activities)),labels =activities,  las=1)

for (i in 1:length(sub_folders_paths) ){
  setwd(paste(sub_folders_paths[i], "/Heat Map View", sep = ""))
  turnsAll <- na.omit(turnsAll)
  dailyFile=dir(pattern="_Vocalization.csv", full.names = FALSE)
...
png(filename ="/Volumes/data/Research/CDB/Progress Monitoring:LENA/Brave Buddies/Plots and Results/Thursday-Voc-Heatmap.png",width=1400, height=600)
...
plot(c(0,length(activities)+1), c(0, 30), type="n" , xlab ="Day", main = "Child Vocalizations On Thursday", cex.main=2, xaxt = "n",ylab = "Average vocalizations per 5 minute segment"  , cex.lab=1.3)
axis(side = 1,at= c(1:length(activities)),labels =activities,  las=1)
