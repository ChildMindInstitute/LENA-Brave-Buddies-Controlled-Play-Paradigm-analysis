#LENA Data Analysis: Audio Level Data
#Jacob Stroud
...
png(filename ="/Volumes/data/Research/CDB/Progress Monitoring:LENA/Brave Buddies/Plots and Results/child-avgdbl.png",width=1400, height=600) #Opens a PNG file#
...
plot(c(0,length(days)+1), c(70, 90), type="n" , xlab ="Day", main = "Child Segment Average Decibel Level", cex.main=2, xaxt = "n",ylab = "dB SPL", cex.lab=1.3)
...
for (i in 1:length(sub_folders_paths) ){
  ...
  dayfiles=dir(pattern=paste(sub_folders[i], "_AudioLevels.csv", sep = ""), full.names = FALSE)
...
#progress
plot(c(0,length(days)), c(-10,10), type="n" , main ="Average Segment dB Level Across Week", cex.main=2, xaxt = "n", xlab=NA, ylab = "Differences in Average Segment dB SPL Compared to Monday", cex.lab=1.3, cex.main=2)
...
png(filename ="/Volumes/data/Research/CDB/Progress Monitoring:LENA/Brave Buddies/Plots and Results/child-peakdbl.png",width=1400, height=600) #Opens a PNG file#
par(mfrow=c(1,2)) #Splits png into 2 sections
...
#Creates a plot for the data
plot(c(0,length(days)+1), c(70, 90), type="n" , xlab ="Day", main = "Average Child Segment Peak dB SPL", cex.main=2, xaxt = "n",ylab = "dB SPL", cex.lab=1.3)
axis(side = 1,at= c(1:length(days)),labels =days,  las=1)
...
#progress
plot(c(0,length(days)), c(-10,10), type="n" , main ="Average Segment Peak dB SPL Across Week", cex.main=2, xaxt = "n", xlab=NA, ylab = "Differences in Peak segment dB SPL Compared to Monday", cex.lab=1.3, cex.main=2)
axis(side = 1,at= 1:3,labels = c( "Tue-Mon", "Wed-Mon", "Thur-Mon"),  las=1)
