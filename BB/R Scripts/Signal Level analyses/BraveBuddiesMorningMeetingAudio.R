#LENA Data Analysis: Morning Meeting Audio Level Data
#Jacob Stroud
...
png(filename ="/Volumes/data/Research/CDB/Progress Monitoring:LENA/Brave Buddies/Plots and Results/MorningMeeting-avgdbl.png",width=1400, height=600) #Opens a PNG file#
...
plot(c(0,length(days)+1), c(70, 90), type="n" , xlab ="Day", main = "Child Segment Average Decibel Level-Morning Meeting", cex.main=2, xaxt = "n",ylab = "dB SPL", cex.lab=1.3)
...
for (i in 1:length(sub_folders_paths) ){
  ...
  dayfiles=dir(pattern=paste(sub_folders[i], "_MorningMeetingAudio.csv", sep = ""), full.names = FALSE)
...
#progress
plot(c(0,length(days)), c(-10,10), type="n" , main ="Average Segment dB Level during Morning Meeting Across Week", cex.main=2, xaxt = "n", xlab=NA, ylab = "Differences in Average Segment dB SPL Compared to Monday", cex.lab=1.3, cex.main=2)
...
for (q in 1:dim(day_diff_avg)[1]){
  ...
  if (identical(sub_folders[q], "M00412434") || identical( sub_folders[q],"M00441664") || identical(sub_folders[q],"M00402344")|| identical(sub_folders[q],"M00440011") || identical( sub_folders[q],"M00494954" ) ){
    groupColor="green"
...
png(filename ="/Volumes/data/Research/CDB/Progress Monitoring:LENA/Brave Buddies/Plots and Results/MorningMeeting-peakdbl.png",width=1400, height=600) #Opens a PNG file#
par(mfrow=c(1,2)) #Splits png into 2 sections
...
#Creates a plot for the data
plot(c(0,length(days)+1), c(70, 90), type="n" , xlab ="Day", main = "Average Child Segment Peak dB SPL-Morning Meeting", cex.main=2, xaxt = "n",ylab = "dB SPL", cex.lab=1.3)
axis(side = 1,at= c(1:length(days)),labels =days,  las=1)
...
#progress
plot(c(0,length(days)), c(-10,10), type="n" , main ="Average Segment Peak dB SPL Across Week-Morning Meeting", cex.main=2, xaxt = "n", xlab=NA, ylab = "Differences in Peak segment dB SPL Compared to Monday", cex.lab=1.3, cex.main=2)
axis(side = 1,at= 1:3,labels = c( "Tue-Mon", "Wed-Mon", "Thur-Mon"),  las=1)
